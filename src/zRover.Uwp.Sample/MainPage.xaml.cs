...
namespace zRover.Uwp.Sample
{
    public sealed partial class MainPage : Page, IActionableApp
    {
        public MainPage()
        {
            ...
            zRover.Uwp.RoverMcp.Log("MainPage", "MainPage initialized");

            // Input diagnostics via zRover logging — visible in get_logs, helps diagnose
            // injection misclicks without requiring further app changes.
            var coreWindow = Windows.UI.Core.CoreWindow.GetForCurrentThread();
            coreWindow.PointerPressed += (s, e) =>
            {
                var pt = e.CurrentPoint;
                zRover.Uwp.RoverMcp.Log("MainPage.Input",
            ...
            this.Loaded += (s, e) =>
                zRover.Uwp.RoverMcp.Log("MainPage.Layout",
        ...
        private void PresetColor_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is string hex)
            {
                ...
                zRover.Uwp.RoverMcp.Log("MainPage.ColorPicker", $"Preset color clicked: #{hex} (R={r}, G={g}, B={b})");
                RedSlider.Value = r;
                GreenSlider.Value = g;
                BlueSlider.Value = b;
            }
        ...
        private void UpdateColorPreview()
        {
            ...
            zRover.Uwp.RoverMcp.Log("MainPage.ColorPicker", $"Color updated: #{r:X2}{g:X2}{b:X2} (R={r}, G={g}, B={b})");
        }
        ...
        {
            zRover.Uwp.RoverMcp.Log("MainPage.TextInput", "Text cleared by user");
            TestTextBox.Text = "";
        ...
        private void ClearInk_Click(object sender, RoutedEventArgs e)
        {
            ...
            zRover.Uwp.RoverMcp.Log("MainPage.InkCanvas", $"Ink cleared ({before} strokes removed)");
        ...
        private void InkMode_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button btn && btn.Tag is string mode)
            {
                ...
                zRover.Uwp.RoverMcp.Log("MainPage.InkCanvas", $"Ink mode changed to: {mode}");
                switch (mode)
                {
                    case "pen":
        ...
        {
            UpdateStrokeCount();
            zRover.Uwp.RoverMcp.Log("MainPage.InkCanvas",
  ...
  ""properties"": {
    ...
    ""value"": {
      ...
      ""maximum"": 255
        ...
        {
            zRover.Uwp.RoverMcp.Log("MainPage.Actions", $"Dispatching action: {actionName} params={parametersJson}");
            try
            {
                ActionResult result;
                switch (actionName)
                ...
                if (result.Success)
                    zRover.Uwp.RoverMcp.Log("MainPage.Actions", $"Action '{actionName}' succeeded");
                else
                    zRover.Uwp.RoverMcp.LogWarn("MainPage.Actions", $"Action '{actionName}' failed: {result.ErrorMessage}");
                return result;
            }
            catch (Exception ex)
            {
                zRover.Uwp.RoverMcp.LogError("MainPage.Actions", $"Action '{actionName}' threw an exception", ex);
