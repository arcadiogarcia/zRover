# deploy-dev.ps1
# Registers zRover.Retriever as a loose-file MSIX package for development.
...
if ($running) {
    Write-Host "Stopping running zRover.Retriever (PID $($running.Id -join ', '))..."
...
$PriCandidates = @(
    ...
    (Join-Path $BuildDir 'zRover.Retriever.pri'),
    (Join-Path (Join-Path $BuildDir $Rid) 'resources.pri'),
    (Join-Path (Join-Path $BuildDir $Rid) 'zRover.Retriever.pri')
)
